<template>
    <div :class="(className ? className + '-wrapper ' : '') + 'autocomplete-wrapper'">
        <input  type="text"
                :name="name"
                :id="id"
                :class="className + ' autocomplete-input ' + validateName"
                ...
                @focus="focus"
                autocomplete="off" />
        <label v-show="labelName" :for="id">{{labelName}}</label>

        <div :class="(className ? className + '-list ' : '') + ' autocomplete panel scale-transition'" v-show="showList">
            <a  v-for="(data, i) in json"
                :class="activeClass(i)"
                href="#"
                @click.prevent="selectList(data)"
                @mousemove="mousemove(i)">
                <!--{{ data[anchor] }} {{ data[label] }}-->
                <div class="anchor">{{ data[anchor] }}</div>
...
<style>
    ...
    .autocomplete a.is-active {
        background-color: whitesmoke;
    }

    .autocomplete a div.anchor {
        color: black !important;
        font-weight: bold;
    }

    .autocomplete a div.label {
  ...
  export default {
    ...
    methods: {
      ...
      selectList(data){
        ...
        window.location.replace('/article/'+clean.slug);
        return false;
